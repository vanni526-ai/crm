...
- [x] 修改数据库会员套餐价格为 ¥39/年
- [x] 后端新增 auth.loginWithToken 接口（App 传 JWT 免登录建立 session）
- [x] 前端 H5 读取 URL token 参数自动登录
- [x] 重新设计支付页面（参考图示：顶部订单信息+账户余额，微信/支付宝/余额/充值四种支付方式卡片，底部确认支付按钮）
- [x] 前端 H5 新增「我已完成支付」按钮，点击后刷新会员状态
- [x] 生成 App 对接 API 文档
- [x] 生成完整 H5 对接方案 MD 文档（供 App 开发团队使用）
