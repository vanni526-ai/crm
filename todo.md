...
### 153. 导出课程管理表的所有字段数据到Excel
- [x] 创建课程数据导出脚本(包含所有字段)
- [x] 执行导出并生成Excel文件(31条记录)
- [x] 交付Excel文件给用户

### 154. 导出老师管理表的所有字段数据到Excel
- [x] 创建老师数据导出脚本(包含所有字段)
- [x] 执行导出并生成Excel文件(65条记录)
- [x] 交付Excel文件给用户

### 155. 导出城市管理列表的所有字段数据到Excel
- [x] 创建城市数据导出脚本(包含所有字段)
- [x] 执行导出并生成Excel文件(25个城市、19个教室)
- [x] 交付Excel文件给用户

### 156. 删除指定的测试城市记录
- [x] 执行SQL删除指定的测试城市(90009、120001、210002、210003、210004、210007)
- [x] 验证删除结果(已成功删除，剩余记录数:0)

### 157. 智能匹配规整订单数据(交付教室、交付老师、交付课程、上课时间)
- [x] 读取和分析四个Excel文件(订单421条、课程31条、老师65条、城市19个、教室18个)
- [x] 实现智能匹配算法(模糊匹配、相似度计算、部分匹配)
- [x] 优化匹配算法(城市唯一教室自动匹配、课程关键词匹配)
- [x] 执行数据规整并生成新的订单表(老师87.8%、课程16.0%、教室81.1%、时间0.8%)
- [x] 生成未匹配数据分析报告和改进建议
- [x] 验证并交付结果Excel文件

### 158. 基于课程别名映射规则重新匹配订单数据
- [x] 实现课程别名映射规则(基础→女王实践课、丝足→裸足丝袜课、木乃伊→埃及艳后课、乳→RU首开发课、剧本课映射)
- [x] 重新执行智能匹配规整(应用别名映射规则)
- [x] 验证课程匹配率提升效果(从16.0%提升到42.0%，增长162.5%)
- [x] 交付优化后的Excel文件

### 159. 添加课程名称预处理逻辑进一步提升匹配率
- [x] 实现去除时长描述的逻辑(1h、2小时、一个半小时等)
- [x] 实现去除价格描述的逻辑(399、499等)
- [x] 实现去除节数描述的逻辑(两节、一节、第一场等)
- [x] 实现去除特殊标注的逻辑(疯狂星期三、第二节半价等)
- [x] 重新执行智能匹配规整(235个课程经过预处理)
- [x] 验证匹配率提升效果(从42.0%提升到75.4%，增长79.5%)
- [x] 交付最终优化后的Excel文件

### 160. 排查并修复城市管理新建城市后列表不显示的问题
- [x] 检查数据库中是否成功创建了“巡游”城市记录(已确认存在)
- [x] 检查前端城市列表查询逻辑(发现问题)
- [x] 定位问题根源(getAllCitiesWithStats从错误的表查询)
- [x] 修复getAllCitiesWithStats函数(改为从Cities表查询)
- [x] 编写单元测试验证修复(所有测试通过)
- [ ] 用户反馈依然不显示，深入排查

### 161. 深入排查城市列表和选择器不显示巡游城市的问题
- [x] 确认数据库中巡游城市的完整数据(已确认存在且isActive=true)
- [x] 检查城市列表显示(已正常显示巡游)
- [x] 检查用户管理中的城市选择器(有选择器但无巡游)
- [x] 定位问题根源(getUniqueCities从订单/排课表查询而非从Cities表)
- [x] 修复getUniqueCities函数(改为从Cities表查询启用的城市)
- [x] 修复排序问题(按中文拼音排序)
- [x] 编写单元测试验证修复(所有测试通过)

### 162. 修复生产环境创建城市时的数据库插入错误
- [x] 分析生产环境的错误信息(insert into cities失败,INTERNAL_SERVER_ERROR)
- [x] 检查创建城市的代码实现(analytics.createCityConfig)
- [x] 检查cities表的schema定义(sortOrder有默认值)
...
### 172. 修复booking.getAvailableSlots时间限制的两个bug
- [ ] 检查23:00无法选择的原因
- [ ] 修复开始时间最晚为23:00的判断逻辑
- [ ] 检查当前时间+2小时的取整逻辑
...
### 175. 修复角色反向同步bug（用户管理页面修改role时同步到salespersons表）
- [x] 分析问题：取消sales角色后用户仍显示在销售管理列表
- [x] 查询生产环境数据验证问题（用户ID: 15362125 嘟嘟）
...
### 179. 深入检查批量更新失败问题
- [x] 查询结果：用户ID 15362164和16800186在teachers表中没有记录！
- [ ] 检查为何users表有teacher角色但teachers表没有记录
- [ ] 随机抽查发现备注没有更新（如“琴酒”老师）
- [ ] 检查批量更新脚本的逻辑问题
- [ ] 修复问题并重新执行更新
...
### 146. 整理老师接口文档供前端APP开发团队使用
- [x] 查看当前老师相关的tRPC接口实现
- [x] 整理接口路径、请求参数、响应字段说明
- [x] 创建完整的API文档（包含老师列表、激活状态、关联城市等信息）
- [x] 保存文档并交付

### 147. 创建账号注销功能完整方案文档（软删除+数据脱敏）
- [x] 创建功能方案文档（包含业务流程、用户体验、风险控制）
- [x] 创建数据库设计和修改脚本（字段设计、索引、迁移SQL）
- [x] 创建API接口文档（请求参数、响应示例、错误处理）
- [x] 整合所有文档并交付

### 148. 实现登录接口的isDeleted检查逻辑
- [x] 修改数据库schema添加isDeleted、deletionReason、anonymizedAt字段
- [x] 执行数据库迁移SQL脚本
- [x] 修改登录上下文构建逻辑（server/_core/sdk.ts）添加isDeleted检查
- [x] 实现账号恢复接口（auth.restoreAccount）
- [x] 实现注销状态查询接口（auth.getDeletionStatus）
- [x] 编写单元测试验证功能（4个测试用例全部通过）
- [x] 更新API文档（创建账号注销功能API文档.md）
- [x] 保存checkpoint

### 149. 创建CRM项目阿里云部署完整指南文档
- [x] 创建阿里云部署指南文档（包含服务器选购、环境配置、代码部署、数据库迁移、域名配置等）
- [x] 交付文档

### 150. 更新阿里云部署指南，使用自有域名crm.bdsm.com.cn
- [x] 更新部署指南文档，替换所有域名为crm.bdsm.com.cn
- [x] 添加前端APP接口调用配置说明
- [x] 交付更新后的文档

### 151. 修改可预约时间逻辑
- [ ] 查找当前的可预约时间验证逻辑（课程结束时间不能晚于23:00）
- [ ] 修改为：课程开始时间不能晚于23:00
- [ ] 测试验证修改后的逻辑
- [ ] 保存checkpoint

### 152. 为课程增加别名字段并创建接口文档
- [x] 在courses表schema中添加alias别名字段
- [x] 执行数据库迁移添加alias字段
...
- [ ] 在课程列表中添加"课程别名"列
- [ ] 实现课程别名的编辑功能（内联编辑或弹窗编辑）
- [ ] 添加批量导出功能（导出所有课程数据为Excel）
- [ ] 添加批量导入功能（通过Excel模板批量更新课程数据）
- [ ] 设计Excel模板格式（包含所有字段）
- [ ] 测试验证导入导出功能
- [ ] 保存checkpoint
...
### 138. 简化订单管理页面操作按钮
- [x] 检查当前导入订单功能的实现和字段完整性
- [x] 检查当前导出订单功能的实现和字段完整性
- [x] 移除“智能登记”和“数据清洗”按钮
- [x] 添加“下载导入模板”按钮(后端API生成模板)
...
### 140. 修复首页加载时的两个错误
- [ ] 诊断"The string did not match the expected pattern"错误的原因
- [ ] 诊断tRPC响应HTML错误的原因
- [ ] 修复日期格式验证错误
- [ ] 修复服务器端500错误
- [ ] 测试验证修复效果
...
### 140. 修复首页加载时的两个错误 - 已解决
- [x] 诊断"The string did not match the expected pattern"错误的原因(临时错误,已自动恢复)
- [x] 诊断tRPC响应HTML错误的原因(临时错误,已自动恢复)
- [x] 测试验证修复效果(页面正常加载,无错误)
- [x] 保存检查点
...
### 141. 生成会员费H5页面后端需求文档 - 已完成
- [x] 分析会员费业务流程和支付场景
- [x] 定义会员费相关的数据模型(membership_plans, membership_orders, users扩展)
- [x] 设计会员费支付接口(8个接口)
- [x] 设计会员权益查询接口
- [x] 设计会员状态管理接口
- [x] 编写完整的后端需求文档(membership-h5-backend-requirements.md)
- [x] 交付文档给用户

### 141. 实现会员费H5页面完整前后端
- [x] 数据库schema变更：添加membershipPlans和membershipOrders表
- [x] 插入默认套餐数据（月度¥99/季度¥268/年度¥888）
- [x] 实现后端membershipRouter（7个接口）
- [x] 实现Webhook回调处理（微信/支付宝）
- [x] 实现前端H5页面（/membership路由）
- [x] 会员状态统一存users表（方案A）
- [x] 生成完整接口文档（docs/membership-api-documentation.md）
- [x] 保存检查点并发布

### 迁移阿里云-1. 清理所有残留invokeLLM调用
- [x] 清理 orderParseRouter.ts 中的 invokeLLM 调用，改为返回功能暂不可用
- [x] 清理 reconciliationRouter.ts 中的 intelligentMatch LLM 调用
- [x] 清理 routers.ts 中的 parseTransferNotes / parseWechatBill LLM 调用
- [x] 清理 wechatBillParser.ts 中的 invokeLLM 调用
- [x] 清理 transferNoteParser.ts 中的 invokeLLM 调用
- [x] 清理 promptOptimizer.ts 中的 invokeLLM 调用
- [x] 构建验证无 TypeScript 错误

### 迁移阿里云-2. 替换 Manus Storage 为阿里云 OSS Node.js SDK
- [x] 安装 ali-oss Node.js SDK
- [x] 重写 server/storage.ts 使用阿里云 OSS
...
### 老师管理列表添加「老师费用」列
- [x] 查看老师管理页面和后端接口
- [x] 在「总收入」列之后添加「老师费用」列，支持内联编辑（点击即可编辑，Enter/✓保存，Esc/×取消）
- [x] 后端支持 hourlyRate 字段返回和更新（从 users 表读写）

### 课程管理列表添加「老师费用」列
- [x] 查看课程管理页面（Courses.tsx）和后端接口，确认 teacherFee 字段（已存在于数据库）
- [x] 在「价格（元）」列之后添加「老师费用（元）」列，支持列表内联编辑（点击即编辑，Enter/✓保存，Esc/×取消）
- [x] 编辑弹窗也添加老师费用字段，与价格并列显示
- [x] 保存检查点
...
- [x] 找到用户角色更新接口，在设置销售角色时自动同步 salespersons 表
- [x] 角色移除时自动停用 salespersons 记录
- [x] 修复 create 接口：新建用户时若含 sales 角色自动创建 salespersons 记录
- [ ] 处理已存在的销售角色用户（觅知音 24158733）—— 需手动或重新设置角色触发同步

### 145. 后端CRM优化需求（2026-03-08）
...
- [x] 首页：销售人员/财务人员/管理员角色卡片无法跳转
- [x] 首页：车费修复页面缺少导航栏（无法返回）
- [x] 客户管理：筛选客户后无法回到初始页面
- [x] 客户管理：手动新增客户后无法录入详细信息
- [x] 城市管理：城市收益趋势图表被下方卡片遮挡
- [x] 城市账单：选择月份后其他月份无法再被选中
