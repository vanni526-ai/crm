...
async function generateJanuaryFinancialReport() {
  console.log("开始生成2026年1月财务统计表...");
  ...
  console.log(`查询到1月订单数: ${januaryOrders.length} 个`);
  ...
  for (const order of januaryOrders) {
  ...
  const summaryData = [
    { "项目": "订单总数", "金额": januaryOrders.length },
  ...
  // 保存文件
  const outputPath = "/home/ubuntu/upload/2026年1月财务统计表.xlsx";
  XLSX.writeFile(workbook, outputPath);

  console.log(`\n✅ 2026年1月财务统计表已生成: ${outputPath}`);
  console.log(`\n📊 统计摘要:`);
  console.log(`订单总数: ${januaryOrders.length}`);
...
generateJanuaryFinancialReport().catch(console.error);
