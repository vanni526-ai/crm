...
- **作废订单识别**: 如果客户名以"作废"开头(如"作废-abc"、"作废 abc"), 请在customerName字段中保留完整的"作废-xxx"格式, 这表示该订单需要作废并删除原订单
  * ⚠️ **非常重要**: 对于作废订单,必须提取文本中的渠道订单号(交易单号)并填入channelOrderNo字段
  * 作废订单示例: "作废 山竹 1.16 19:30-21:30 基础局+医生 米妮上 (广州巡游) . 1000订金已付 3200尾款未付 给老师1260 交易单号4200002917202601132625805238"
    → customerName="作废 山竹", channelOrderNo="4200002917202601132625805238", 其他字段正常填写
- 例如：
...
3. 客户名 - 客户的名字
   ⚠️ **客户名识别规则**(非常重要,必须严格遵守):
   * **典型订单格式**: 销售名 + 日期 + 时间 + 课程 + 老师名 + 客户名 + 金额信息
   * **示例1**: "山竹 1.24 13:00-15:00基础局+裸足丝袜课  酥酥上 HxL （宁波上）订金1200已付" 
     → 销售名="山竹", 老师="酥酥", **客户名="HxL"** (不是"酥酥"!)
   * **示例2**: "奈落 1.23 18:00-19:00 埃及艳后课1h yy上 lsc 1500全款已付"
     → 销售名="奈落", 老师="yy", **客户名="lsc"** (不是"yy"!)
   * **示例3**: "土豆1.23 14.00-15.00 sp 1h yy上（天津）某市民 定金750已付"
     → 销售名="土豆", 老师="yy", **客户名="某市民"** (不是"yy"!)
  * **示例4**: "山竹 1.24 13:00-16:00 足下恩典+屈辱  yy上（天津上） 剑桃 6000全款已付"
     → 销售名="山竹", 老师="yy", **客户名="剑桃"** (在括号之后,不是"yy"!)
   * **位置规则**: 客户名通常在老师名之后、金额信息之前
    - 如果老师名后面有括号(如"yy上（天津上）"),客户名在括号之后
    - 如果老师名后面没有括号,客户名紧跟在老师名之后
   * **排除规则**: 客户名**不能**是:
     - 老师名(如"酥酥"、"yy"、"米妮"、"声声"等)
     - 课程名(如"高定网T"、"LEC4"、"基础局"、"埃及艳后"等)
     - 城市名(如"天津"、"宁波"、"上海"等)
     - 销售人员名(如"山竹"、"奈落"、"土豆"等)
   * **如果无法确定客户名,则留空**
...
18. 渠道订单号 - 交易单号/渠道订单号,常见表达:
   ...
   * "交易编号XXXXXXXX" (如"交易编号A2026012119504910049723")
   * "支付宝商户编号XXXXXXXX" (如"支付宝商户编号A2026012213174510046217")
   * "商家订单号XXXXXXXX" (如"商家订单号A2026012211544010041453")
   * ⚠️ **非常重要**: 必须将渠道订单号提取到channelOrderNo字段中,不要放在notes字段里!
   * ⚠️ **重要**: 如果没有明确提到渠道订单号,则必须留空(空字符串""),不要填充"1.1"、"1.2"等占位符数字
   * 渠道订单号通常是15位以上的长数字串(如微信支付宝订单号),不会是"1.1"这样的简单数字
   * 支付宝订单号格式: A开头+数字,共25-28位 (如A2026012119504910049723)
   * 微信支付订单号格式: 纯数字,共28-32位 (如4200002971202512209215930344)
19. 备注 - 保存该订单的完整原始文本信息(从销售人员名开始到该条消息结束的所有内容)
   ⚠️ 非常重要 - 订单边界隔离
   * **只保存当前订单的原始文本,不要混入其他订单的信息**
   * ⚠️ **注意**: 渠道订单号应该提取到channelOrderNo字段,不要重复放在notes字段中
...
- **客户名识别最佳实践**(⚠️ 非常重要):
  * **步骤1**: 先识别销售名(第一个词)
  * **步骤2**: 再识别老师名(通常带"XX上"后缀,如"酥酥上"、"yy上")
  * **步骤3**: 客户名在老师名之后、金额信息之前的那个词
  * **步骤4**: 验证客户名不是老师名、课程名、城市名、销售名
  * **如果以上步骤无法确定客户名,则留空**
  * **常见错误**: 把老师名误识别为客户名(如把"酥酥"误识别为客户名,实际客户名是"HxL")
  * **正确示例**: "山竹 1.24 13:00-15:00基础局+裸足丝袜课 酥酥上 HxL (宁波上)订金1200已付"
    → 销售名="山竹" → 老师名="酥酥" → **客户名="HxL"** → 城市="宁波" → 金额="1200"
- 账户余额提取规则:
