import { View, Text, ScrollView, TouchableOpacity, ActivityIndicator, RefreshControl, Pressable, TextInput } from "react-native";
...
type FilterType = 'all' | 'undelivered' | 'delivered';

// 优化9: 订单卡片组件 - React.memo
const SalesOrderCard = memo(function SalesOrderCard({
  ...
  return (
    <View className="bg-surface rounded-xl p-4">
      ...
      {order.deliveryTeacher && (
        <Text className="text-xs text-muted mt-1">老师: {order.deliveryTeacher}</Text>
      )}
    </View>
  );
});
...
export default function SalesOrdersScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setOrders(list.sort((a: any, b: any) => (b.createdAt || '').localeCompare(a.createdAt || '')));
    } catch (error) {
      errorLogger.error('SalesOrders', 'Load failed', error);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
        ...
        <View className="p-6 gap-4">
          ...
          {loading ? (
            ...
            <View className="items-center justify-center py-20">
              ...
              <Text className="text-muted mt-4">{debouncedSearch ? '未找到匹配的订单' : '暂无订单'}</Text>
            </View>
          ) : (
