...
import { User, Phone, Mail, Lock, LogOut, ChevronRight, Shield } from "lucide-react";
...
export default function UserProfile() {
  ...
  const u = user as any;
  const userName = u?.nickname || u?.name || "用户";
  const userRoles = (u?.roles || u?.role || "user").toString().split(",");
...
function PasswordModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-end justify-center">
      <div className="w-full max-w-lg bg-[#16161f] rounded-t-3xl p-6 pb-10 animate-slide-up">
        ...
        <div className="space-y-4">
          <div>
            <label className="text-slate-400 text-sm mb-1 block">旧密码</label>
            <input
              type="password"
              value={oldPassword}
          ...
          <div>
            <label className="text-slate-400 text-sm mb-1 block">新密码</label>
            <input
              type="password"
              value={newPassword}
          ...
          <div>
            <label className="text-slate-400 text-sm mb-1 block">确认新密码</label>
            <input
              type="password"
