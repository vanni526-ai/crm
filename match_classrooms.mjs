...
import { orders, cities, classrooms } from './drizzle/schema.ts';
...
const unmatchedOrders = await db
  ...
  .where(and(
    ...
    eq(orders.deliveryClassroomId, null)
  ));
...
if (matchResults.length > 0) {
  ...
  for (const result of matchResults) {
    await db
      .update(orders)
      .set({ deliveryClassroomId: result.classroomId })
